...
const routeSlug = route.params.district as string
const slug = routeSlug.toLowerCase()

if (routeSlug !== slug) {
  await navigateTo({ path: `/${slug}`, query: route.query, hash: route.hash }, { redirectCode: 301 })
}
...
const hiddenSections = computed(() => {
  ...
  const visible = ((cal as any)?.visibleSections ?? (cal as any)?.meta?.visibleSections ?? []) as string[]
...
const verificationBadgeReviewerName = computed(() =>
  String((cal as any)?.verificationBadgeReviewerName ?? (cal as any)?.meta?.verificationBadgeReviewerName ?? '')
)
const verificationBadgeReviewerHref = computed(() =>
  String((cal as any)?.verificationBadgeReviewerHref ?? (cal as any)?.meta?.verificationBadgeReviewerHref ?? '/author')
)
...
const alternateCalendarsNoticeBeforeKeyDates = computed(() =>
  String((cal as any)?.alternateCalendarsNoticePosition ?? (cal as any)?.meta?.alternateCalendarsNoticePosition ?? '') === 'beforeKeyDates'
)
...
const otherCalendarsBeforeKeyDates = computed(() =>
  ((cal as any)?.otherCalendarsPosition ?? (cal as any)?.meta?.otherCalendarsPosition) === 'beforeKeyDates'
)
...
const breakSummaryOverrides = computed<Record<string, { name?: string; start?: string; end?: string; note?: string; durationLabel?: string }>>(() =>
  ((cal as any)?.breakSummaryOverrides ?? (cal as any)?.meta?.breakSummaryOverrides ?? {}) as Record<string, { name?: string; start?: string; end?: string; note?: string; durationLabel?: string }>
)
...
const yearComparisonAfterAllDates = computed(() =>
  ((cal as any)?.yearComparisonPosition ?? (cal as any)?.meta?.yearComparisonPosition) === 'afterAllDates'
)
...
const gradingPeriodsBeforeCalendarExport = computed(() =>
  ((cal as any)?.gradingPeriodsPosition ?? (cal as any)?.meta?.gradingPeriodsPosition) === 'beforeCalendarExport'
)
...
const resolvedJumpNavigation = computed(() => {
  ...
  const customTargets = (position: string) => customSections.value
    .filter(section => section.position === position || (position === 'afterAbout' && !section.position))
    .map(section => `#${section.id}`)
  const targets = [
    ...customTargets('afterVerification'),
    ...((otherCalendarsBeforeKeyDates.value || (otherCalendarsAfterKeyDates.value && hiddenSections.value.has('keyDateCards')))
      ? ['#other-calendars', ...customTargets('afterOtherCalendars')]
      : []),
    ...(hiddenSections.value.has('keyDateCards') ? customTargets('afterAlternateCalendarsNotice') : []),
    '#key-dates',
    ...(otherCalendarsAfterKeyDates.value && !hiddenSections.value.has('keyDateCards')
      ? ['#other-calendars', ...customTargets('afterOtherCalendars')]
      : []),
    ...(!hiddenSections.value.has('keyDateCards') ? customTargets('afterAlternateCalendarsNotice') : []),
...
if (!isStatePage && district.value) {
  ...
  const sourceCalendarId = String((cal as any)?.sourceCalendarId ?? (cal as any)?.meta?.sourceCalendarId ?? 'source-calendar').replace(/^#/, '')
  ...
  const schemaDatasetDateModified = (cal as any)?.schemaDatasetDateModified ?? (cal as any)?.meta?.schemaDatasetDateModified ?? pageDateModified
  ...
  const calendarIcsUrl = district.value && cal
    ...
    : ''
  const hideIcsFromDatasetDistribution = Boolean((cal as any)?.hideIcsFromDatasetDistribution || (cal as any)?.meta?.hideIcsFromDatasetDistribution)
  const calendarTrackDistributions = hideIcsFromDatasetDistribution
    ? []
    : (((cal as any)?.calendarTrackDownloads ?? (cal as any)?.meta?.calendarTrackDownloads ?? []) as Array<{ id?: string; label?: string; description?: string }>)
        ...
        .map(track => ({
          ...
          contentUrl: `https://myschooldates.com/calendars/${district.value!.slug}-${cal!.schoolYear}-${track.id}.ics`,
        }))
  ...
  const datasetEntity = hideDatasetSchema ? null : {
    ...
    ...(!hideIcsFromDatasetDistribution || includePrintablePdfInDatasetDistribution ? {
      distribution: [
        ...calendarTrackDistributions,
        calendarIcsUrl && !hideIcsFromDatasetDistribution ? {
          '@type': 'DataDownload',
  ...
  const keyDateItemListId = String(
    (cal as any)?.schemaKeyDateItemListId ?? (cal as any)?.meta?.schemaKeyDateItemListId ?? 'key-dates',
  ).replace(/^#/, '')
  ...
  const webPageParts = [
    ...
    ...(!hideItemListSchema && summaryItemListEvents.length ? [{ '@id': `${canonicalUrl}#${keyDateItemListId}` }] : []),
  ...
  const webPageMainEntity = webPageMainEntityMode === 'none'
    ...
    : webPageMainEntityMode === 'keyDates'
      ? (!hideItemListSchema && summaryItemListEvents.length ? { '@id': `${canonicalUrl}#${keyDateItemListId}` } : null)
      : webPageMainEntityMode === 'dataset'
        ...
        : datasetEntity
          ...
          : !hideItemListSchema && summaryItemListEvents.length
            ? { '@id': `${canonicalUrl}#${keyDateItemListId}` }
            : null
  const webPageEntity = {
    '@type': 'WebPage',
...
<template>
  ...
  <template v-if="isStatePage">
      <main class="state-calendar-page site-page-shell py-8 space-y-8">
        ...
        <!-- Quick Answer -->
        <div v-if="statePageData" id="state-quick-answer" class="state-panel state-panel--accent p-6 scroll-mt-24">
          <h2 class="text-lg font-semibold mb-2">{{ statePageData.quickAnswerTitle ?? `Quick Answer: ${matchedStateName} School Calendar ${stateCurrentYear}` }}</h2>
          <p class="text-sm leading-relaxed text-rds-ink-muted">
  ...
  <template v-else-if="district && cal">
      <main class="district-calendar-page py-8">
        ...
        <section class="district-page-section district-hero">
          ...
          <p class="district-hero__source mt-3 text-sm">
            ...
            <template v-if="!((cal as any).hideHeroDownloadLink || (cal as any).meta?.hideHeroDownloadLink)">
              ...
              <a
                ...
                class="rds-link inline-flex min-h-11 items-center underline"
              >
          ...
          <div v-if="heroCtas.length" class="mt-4 flex flex-wrap gap-2">
            <a
              ...
              :target="!cta.download && cta.isExternal ? '_blank' : undefined"
              :rel="!cta.download && cta.isExternal ? 'noopener' : undefined"
              class="inline-flex min-h-11 items-center justify-center px-4 py-2 text-sm font-semibold"
          ...
          <div v-if="heroLinks.length" class="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm">
            <a
              ...
              class="rds-link font-medium"
            >
              {{ link.label }}
              <span v-if="link.isExternal" class="sr-only">(opens in a new tab)</span>
            </a>
          </div>
          <div v-if="heroQuickDates.length" id="quick-answer" class="district-hero__quick-dates mt-4 scroll-mt-24 p-4">
          ...
          >
            <div v-if="!((cal as any).hideHeroReviewedField || (cal as any).meta?.hideHeroReviewedField)">
              <dt class="inline font-semibold uppercase tracking-wide text-[#6b645c]">{{ (cal as any).heroReviewedLabel ?? (cal as any).meta?.heroReviewedLabel ?? 'Reviewed' }}</dt>
            ...
            <div v-if="!((cal as any).hideHeroUpdatedField || (cal as any).meta?.hideHeroUpdatedField)">
              <dt class="inline font-semibold uppercase tracking-wide text-[#6b645c]">{{ (cal as any).heroUpdatedLabel ?? (cal as any).meta?.heroUpdatedLabel ?? 'Updated' }}</dt>
          ...
          <div
            ...
            class="district-hero__verification mt-5 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium"
            :class="isEstimated
          ...
          >
            ...
            <template v-if="!isEstimated && verificationBadgeReviewerName">
        ...
        <!-- Alternate calendars notice before key dates -->
        <div v-if="(cal as any)?.alternateCalendars?.length && !hideAlternateCalendarsNotice && alternateCalendarsNoticeBeforeKeyDates" class="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3">
          ...
          <div class="divide-y divide-gray-100">
            ...
            >
              <div class="flex items-start gap-2.5 min-w-0">
                <span
                  class="mt-0.5 inline-flex flex-shrink-0 justify-center rounded-lg px-2 py-0.5 text-center text-xs font-medium whitespace-nowrap sm:w-36"
